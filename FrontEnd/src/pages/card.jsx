...
<div>
  {
    ...
    return(
      <div key={index} className=' py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr]  items-center gap-4 '>
          ...
          <input onChange={(e) => e.target.value === '' || e.target.value === '0' ? null :updateQuantity(item._id,item.size,Number(e.target.value))} type="number" min={1} max={10} defaultValue={item.quantity} className='border max-w-10 sm:max-w-20 px-1  sm:px-2 py-1 ' />
...
<button onClick={() => navigate('/placeorder')} className=' bg-black text-white text-sm my-10 mx-9 px-8 py-3'>PROCEED TO CHECKOUT</button>
</div>
</div>
</div>
    </div>
